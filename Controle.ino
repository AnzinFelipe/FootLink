...
Joystick_ Joystick(JOYSTICK_DEFAULT_REPORT_ID,JOYSTICK_TYPE_GAMEPAD, 
  15, 0,                  // Quantidade de botões, Numero de Hat Switch
  true, true, true,     // Eixos X e Y no joystick, mas não o eixo Z
  true, false, false,   // Sem Rx, Ry, ou Rz
  false, false,          // Sem rudder ou throttle
  false, false, false);  // Sem acelerador, freio, ou direção
  
void setup() {
  Serial.begin(9600);
  pinMode(Joystick_VRX2, INPUT); // Setando como entrada porque este pino deverá ler o valor do joystick
  pinMode(Joystick_VRY2, INPUT); // Setando como entrada porque este pino deverá ler o valor do joystick
...
void loop() {
  ...
  Joystick.setZAxis(map(analogRead(Joystick_VRX2),0,1023,-127,127)); // Utilizando a função Map para que o sinal original de 0 1023 seja adequado a -127 a 127
  Joystick.setRxAxis(map(analogRead(Joystick_VRY2),0,1023,127,-127)); // Utilizando a função Map como no exemplo anterior, mas utilizando a vantagem da inversão
